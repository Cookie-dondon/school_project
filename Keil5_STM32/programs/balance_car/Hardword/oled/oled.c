...
//mode:Ä£Ê½£¬0-²»ÏÔÊ¾¸ºÊý 1-ÏÔÊ¾¸ºÊý
void OLED_ShowNum(u8 x,u8 y,int32_t num,u8 len,u8 size2 ,u8 mode)
{         	
	u8 t,temp;
	u8 enshow=0;
	int32_t NUM_show;
	if(mode==1)
	{
		if(num<0)
		{
			OLED_ShowChar(x+(size2/2)+5,y,'-',size2);
			NUM_show=-num;
		}else
		{
			OLED_ShowChar(x+(size2/2)+5,y,'+',size2);
	...
	for(t=0;t<len;t++)
	{
		temp=(NUM_show/oled_pow(10,len-t-1))%10;
		if(enshow==0 && t<(len-1))
		{
			if(temp==0)
			{
...
{ 	
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9|GPIO_Pin_8;	 //PD3,PD6ÍÆÍìÊä³ö  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //ÍÆÍìÊä³ö
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//ËÙ¶È50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //³õÊ¼»¯GPIOD8,9
 	GPIO_SetBits(GPIOD,GPIO_Pin_9|GPIO_Pin_8);	//PD3,PD6 Êä³ö¸ß
